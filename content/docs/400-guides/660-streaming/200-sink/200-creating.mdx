---
title: Sinkの作成
excerpt: ストリーム要素を処理するための強力なSinkの構築方法を学びます。`head`、`last`、`count`、`sum`、`take`、`drain`、`timed`、`forEach`などの一般的なコンストラクタを探求し、成功と失敗からSinkを作成する方法を発見します。`collectAll`、`collectAllToSet`、`collectAllToMap`、`collectAllN`、`collectAllWhile`、`collectAllToSetN`、`collectAllToMapN`を使用した収集戦略、`foldLeft`、`fold`、`foldWeighted`、`foldUntil`などの折りたたみ技術に飛び込みます。
bottomNavigation: pagination
---

ストリームの世界では、Sinkはストリームの要素を消費して処理するために使用されます。ここでは、特定のタスクのためにSinkを作成できる一般的なSinkコンストラクタを紹介します。

## 一般的なコンストラクタ

### head

`head`Sinkは、ストリームの最初の要素をキャプチャするSinkを作成します。ストリームが空の場合、それは`None`を返します。
