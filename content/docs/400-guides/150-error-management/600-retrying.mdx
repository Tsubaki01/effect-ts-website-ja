...
`while`または`until`オプションを使用して条件を指定することで、エフェクトがどのようにリトライされるかを管理できます。
...
// 各呼び出しで異なるエラーをシミュレートするエフェクトを定義する
const action = Effect.failSync(() => {
  console.log(`Action called ${++count} time(s)`)
  return `Error ${count}`
})

// 特定の条件が満たされるまでアクションをリトライする
...
<Info>
  エラーではなく成功した結果に基づいて条件を設定するには、
  [Effect.repeat](../scheduling/repetition#repeating-based-on-a-condition)の使用を検討してください。
</Info>
