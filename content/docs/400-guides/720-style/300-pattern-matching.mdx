...
- `Match.type<{ a: number } | { b: string }>(){:ts}`: これは、`{ a: number }`または`{ b: string }`の型を持つオブジェクトに対する`Matcher`を作成します。
- `Match.when({ a: Match.number }, (_) => _.a){:ts}`: これは、プロパティ`a`に数値を含むオブジェクトをマッチさせる条件を設定します。マッチした場合、プロパティ`a`の値を返します。
- `Match.when({ b: Match.string }, (_) => _.b){:ts}`: これは、プロパティ`b`に文字列を含むオブジェクトをマッチさせる条件を設定します。マッチした場合、プロパティ`b`の値を返します。
- `Match.exhaustive{:ts}`: これは、すべての可能なケースが考慮され、マッチされることを保証する関数です。これにより、見落とされる可能性のあるシナリオを防ぐのに役立ちます。
